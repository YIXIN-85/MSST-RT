...
def get_one_actor_points(body_data, num_frames):
    ...
    # if end-start < length:
    #     joints[end+1: 2*end-start+2]=body_data['joints'].reshape(-1, 75)
    #     colors[end+1: 2*end-start+2, 0] = body_data['colors']
...
def get_raw_denoised_data():
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        # if bodies_data['num_frames'] >= length:
        #     performer.append(int(p))
        #     label.append(int(ske_name[17:20]))
        #     # print(int(p))
        #     i += 1
        # if ske_name == 'S029C003P088R001A066':
        #     print("bodies_data:", bodies_data)

        num_bodies = len(bodies_data['data'])
        # if bodies_data['num_frames'] < length:
        #     print("throw the short sample:", ske_name, "len:", bodies_data['num_frames'])
        #     continue
        ...
        #     frames_error.append(num_frames)
        #     print("ske_name:", ske_name)


    # performer = np.asarray(performer, dtype=np.int)
    # label = np.asarray(label, dtype=np.int)
    # frames_error = np.asarray(frames_error, dtype=np.int)
    # np.savetxt('performer2.txt', performer, fmt='%d')
    # np.savetxt('label2.txt', label, fmt='%d')
    # np.savetxt('frames_error.txt', frames_error, fmt='%d')
