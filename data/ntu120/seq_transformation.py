...
performer_file = osp.join(stat_path, 'performer2.txt')
...
def split_dataset(skes_joints, label, performer, setup, camera, evaluation, save_path):
    train_indices, test_indices = get_indices(performer, setup, camera, evaluation)
    ...
    val_indices = val_indices.astype('int64')
...
def get_indices(performer, setup, camera, evaluation='CSub'):
    test_indices = np.empty(0)
    train_indices = np.empty(0)

    if evaluation == 'CV':  # Cross View (Camera IDs)
        train_ids = [2, 3]
        test_ids = 1
...
if __name__ == '__main__':
    ...
    label = np.loadtxt(label_file, dtype=np.int) - 1  # action label: 0~120
    ...
    for evaluation in evaluations:
        split_dataset(skes_joints, label, performer, setup, camera, evaluation, save_path)
