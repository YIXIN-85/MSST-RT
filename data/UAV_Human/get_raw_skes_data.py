...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    print('Reading data from %s' % ske_file[25:])
    ...
    for f in range(num_frames):
        ...
        valid_frames += 1
        joints = np.zeros((num_bodies, 17, 2), dtype=np.float32)
        ...
        for b in range(num_bodies):
            ...
            for j in range(num_joints):
                ...
                joints[b, j, :] = np.array(temp_str[:2], dtype=np.float32)
                current_line += 1

            if b not in bodies_data:  # Add a new body's data
                body_data = dict()
            ...
            bodies_data[b] = body_data  # Update bodies_data
...
def get_raw_skes_data():
    ...
    skes_train_name = np.loadtxt(skes_train_name_file, dtype=str)
    skes_test_name = np.loadtxt(skes_test_name_file, dtype=str)

    train_num_files = skes_train_name.size
    test_num_files = skes_test_name.size
    print('Found %d train available skeleton files.' % train_num_files)
    print('Found %d test available skeleton files.' % test_num_files)

    raw_train_skes_data = []
    raw_test_skes_data = []
    train_frames_cnt = np.zeros(train_num_files, dtype=np.int)
    test_frames_cnt = np.zeros(test_num_files, dtype=np.int)

    for (idx, ske_name) in enumerate(skes_train_name):
        bodies_data = get_raw_bodies_data(skes_train_path, ske_name, frames_train_drop_skes, frames_drop_logger)
        raw_train_skes_data.append(bodies_data)
        train_frames_cnt[idx] = bodies_data['num_frames']
        if (idx + 1) % 1000 == 0:
            print('Processed: %.2f%% (%d / %d)' % \
                  (100.0 * (idx + 1) / train_num_files, idx + 1, train_num_files))

    with open(save_train_data_pkl, 'wb') as fw:
        pickle.dump(raw_train_skes_data, fw, pickle.HIGHEST_PROTOCOL)

    np.savetxt(osp.join(save_path, 'raw_data', 'train_frames_cnt.txt'), train_frames_cnt, fmt='%d')

    for (idx, ske_name) in enumerate(skes_test_name):
        bodies_data = get_raw_bodies_data(skes_test_path, ske_name, frames_test_drop_skes, frames_drop_logger)
        raw_test_skes_data.append(bodies_data)
        test_frames_cnt[idx] = bodies_data['num_frames']
        if (idx + 1) % 1000 == 0:
            print('Processed: %.2f%% (%d / %d)' % \
                  (100.0 * (idx + 1) / test_num_files, idx + 1, test_num_files))

    with open(save_test_data_pkl, 'wb') as fw:
        pickle.dump(raw_test_skes_data, fw, pickle.HIGHEST_PROTOCOL)

    np.savetxt(osp.join(save_path, 'raw_data', 'test_frames_cnt.txt'), test_frames_cnt, fmt='%d')

    print('Saved raw train bodies data into %s' % save_train_data_pkl)
    print('Total train frames: %d' % np.sum(train_frames_cnt))
    print('Saved raw test bodies data into %s' % save_test_data_pkl)
    print('Total test frames: %d' % np.sum(test_frames_cnt))

    with open(frames_train_drop_pkl, 'wb') as fw:
        pickle.dump(frames_train_drop_skes, fw, pickle.HIGHEST_PROTOCOL)

    with open(frames_test_drop_pkl, 'wb') as fw:
        pickle.dump(frames_test_drop_skes, fw, pickle.HIGHEST_PROTOCOL)
...
if __name__ == '__main__':
    ...
    skes_train_name_file = osp.join(stat_path, 'skes_train_available_name.txt')
    skes_test_name_file = osp.join(stat_path, 'skes_test_available_name.txt')
    save_train_data_pkl = osp.join(save_path, 'raw_data', 'raw_skes_train_data.pkl')
    save_test_data_pkl = osp.join(save_path, 'raw_data', 'raw_skes_test_data.pkl')
    frames_train_drop_pkl = osp.join(save_path, 'raw_data', 'frames_train_drop_skes.pkl')
    frames_test_drop_pkl = osp.join(save_path, 'raw_data', 'frames_test_drop_skes.pkl')

    frames_drop_logger = logging.getLogger('frames_drop')
    frames_drop_logger.setLevel(logging.INFO)
    frames_drop_logger.addHandler(logging.FileHandler(osp.join(save_path, 'raw_data', 'frames_drop.log')))
    frames_train_drop_skes = dict()
    frames_test_drop_skes = dict()
