...
root_path = './'
raw_train_data_file = osp.join(root_path, 'raw_data', 'raw_skes_train_data.pkl')
raw_test_data_file = osp.join(root_path, 'raw_data', 'raw_skes_test_data.pkl')
...
def denoising_by_spread(ske_name, bodies_data):
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        if len(bodies_data) == 1:
            break
        valid_frames = get_valid_frames_by_spread(body_data['joints'].reshape(-1, 17, 2))
        ...
        if ratio >= noise_spr_thres2:  # 0.69754
            ...
            joints = body_data['joints'].reshape(-1, 17, 2)[valid_frames]
            body_data['motion'] = min(motion, np.sum(np.var(joints.reshape(-1, 2), axis=0)))
...
def get_one_actor_points(body_data, num_frames):
    ...
    For joints, each frame contains 34 X-Y coordinates.
    """
    joints = np.zeros((num_frames, 34), dtype=np.float32)
    start, end = body_data['interval'][0], body_data['interval'][-1]
    joints[start:end + 1] = body_data['joints'].reshape(-1, 34)
...
def remove_missing_frames(ske_name, joints):
    ...
    if joints.shape[1] == 68:  # DEBUG
        missing_indices_1 = np.where(joints[:, :34].sum(axis=1) == 0)[0]
        missing_indices_2 = np.where(joints[:, 34:].sum(axis=1) == 0)[0]
...
def get_two_actors_points(bodies_data):
    """
    Get the first and second actor's joints positions.
    ...
    # print("************************************************************************")
    # print("bodies_data:", bodies_data['data'])
    bodies_info = get_bodies_info(bodies_data['data'])
    # print("bodies_info:", bodies_info)

    # print("bodies_data:", bodies_data)
    ...
    if len(bodies_data) == 1:  # Only left one actor after denoising
        ...
        while len(bodies_data) > 0:
            ...
            if min(end1, end) - max(start1, start) <= 0:  # no overlap with actor1
                joints[start:end + 1, :34] = actor['joints'].reshape(-1, 34)
...
def get_raw_denoised_data():
    ...
    # with open(raw_train_data_file, 'rb') as fr:  # load raw skeletons data
    #     raw_skes_data = pickle.load(fr)

    with open(raw_test_data_file, 'rb') as fr:  # load raw skeletons data
        raw_skes_data = pickle.load(fr)
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        if num_bodies == 1:  # only 1 actor
            ...
            joints = get_two_actors_points(bodies_data)
    ...
    # raw_skes_joints_pkl = osp.join(save_path, 'raw_train_denoised_joints.pkl')
    raw_skes_joints_pkl = osp.join(save_path, 'raw_test_denoised_joints.pkl')
    ...
    # np.savetxt(osp.join(save_path, 'train_frames_cnt.txt'), frames_cnt, fmt='%d')
    np.savetxt(osp.join(save_path, 'test_frames_cnt.txt'), frames_cnt, fmt='%d')
